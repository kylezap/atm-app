...
The current codebase is partly scaffolded. The web app, API wiring, shared contracts, and tests are in place, but the external PIN API client and withdrawal-processing logic are still stubbed with `NotImplementedError`.
...
|   |   |   |   |-- withdrawal-service.ts # Withdrawal sequence stub
...
- Implemented: monorepo wiring, React UI, Express routing, shared request/response contracts, config parsing, and basic tests.
- Stubbed: `pin-api-client.ts`, `withdrawal-service.ts`, and `note-dispenser.ts`.
